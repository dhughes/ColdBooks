...
<cfcomponent
	displayname="validateNumeric"
	output="false"
	hint="Numeric validation rule."
	extends="validator">
	
	<!--- ------------------------------------------------------------ --->
	<!--- constructor --->

	<cffunction name="init" access="public" returntype="validateNumeric"
	...
	<!--- 
		...
		description:	Check to see if the data value contains only numeric characters and optional special characters (-,.).
	--->
	<cffunction name="validate" access="public" output="false" returntype="string"
		hint="Check to see if the data value contains only numeric characters and optional special characters (-,.).">
		...
		<cfif NOT isSimpleValue( arguments.data ) >
			<cfthrow type="validat.invalidData" message="validat: The validation rule 'validateNumeric' only accepts simple data types." />
		</cfif>
		
		<!--- remove any comma seperators from the data value --->
		<cfset dataValue = replace( arguments.data, ",", "", "all") >
		
		<!--- check to see if the data value contains only numeric characters and optional special characters (-.). --->
		<cfif reFind( "^(\-)?\d*(\.\d+)?$", dataValue ) >

			<cfreturn true />
			
		</cfif> <!--- end: check to see if the data value contains only numeric characters optional special characters (-.). --->
