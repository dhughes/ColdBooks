...
<cfcomponent
	displayname="validatePostalCode"
	output="false"
	hint="US Postal (Zip) Code validation rule."
	extends="validator">
	
	<!--- ------------------------------------------------------------ --->
	<!--- constructor --->

	<cffunction name="init" access="public" returntype="validatePostalCode"
	...
	<!--- 
		...
		description:	Check to see if the data value contains a valid US postal (zip) code string.
	--->
	<cffunction name="validate" access="public" output="false" returntype="string"
		hint="Check to see if the data value contains a valid US postal (zip) code string.">
		...
		<cfif NOT isSimpleValue( arguments.data ) >
			<cfthrow type="validat.invalidData" message="validat: The validation rule 'validatePostalCode' only accepts simple data types." />
		</cfif>
		
		<!--- check to see if the data value contains a valid US postal (zip) code string. --->
		<cfif reFind( "^\d{5}$|^\d{5}-\d{4}$", arguments.data ) >

			<cfreturn true />
			
		</cfif> <!--- end: check to see if the data value contains a valid US postal (zip) code string. --->
