...
<cfcomponent
	displayname="validateSame"
	output="false"
	hint="Same string validation rule."
	extends="validator">
	
	<!--- ------------------------------------------------------------ --->
	<!--- constructor --->

	<cffunction name="init" access="public" returntype="validateSame"
	...
	<!--- 
		...
		description:	Check to see if the data values are same from each other.
	...
	<cffunction name="validate" access="public" output="false" returntype="string"
		hint="Check to see if the data values are same from each other.">
		...
		<cfif NOT isStruct( arguments.data ) >
			<cfthrow type="validat.invalidData" message="validat: The validation rule 'validateSame' only accepts a structure data type." />
		</cfif>
		...
		<cfif structKeyExists( arguments.args, 'dataValue1' ) AND structKeyExists( arguments.data, arguments.args.dataValue1 ) AND
			...
			<!--- check if data values are same --->
			<cfif arguments.data[arguments.args.dataValue1] EQ arguments.data[arguments.args.dataValue2] >
				<cfreturn true />
			</cfif>
		...
		<cfelseif listLen( structKeyList( arguments.data ) ) >
			...
			<!--- check if data values are same --->
			<cfif arguments.data[listFirst( structKeyList( arguments.data ) )] EQ arguments.data[listLast( structKeyList( arguments.data ) )] >
				<cfreturn true />
			</cfif>
