...
<cfcomponent
	...
	<cffunction name="init" access="public" returntype="validateEmail"
	...
	<cffunction name="validate" access="public" output="false" returntype="string"
		...
		<cfif NOT isSimpleValue( arguments.data ) >
			<cfthrow type="validat.invalidData" message="validat: The validation rule 'validateEmail' only accepts simple data types." />
		</cfif>
		
		<!--- check to see if the data value contains only alpha characters ( A-Z, a-z ). --->
		<cfif reFindNoCase( "['_a-z0-9-]+(\.['_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*\.(([a-z]{2,3})|(aero|coop|info|museum|name|jobs|travel))", arguments.data )
				AND ( len( listGetAt( arguments.data, 1, "@" ) ) LTE 64 ) 
				AND ( len( listGetAt( arguments.data, 2, "@" ) ) LTE 255 ) >
			<cfreturn true />
		</cfif>
